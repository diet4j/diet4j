        ...
        ModuleRegistry
{
    ...
    public final Module [] determineDependencies(
        ...
        throws
            ...
            ModuleResolutionException
    {
        resolve( theModule.getModuleMeta(), true );
            ...
            ClassLoader parentClassLoader )
    {
        ...
        } catch( MalformedURLException | ModuleNotFoundException | ModuleResolutionException ex ) {
            ex.printStackTrace();
        }
        return ret;
    }
    ...
    /**
     * Dump the AbstractModuleRegistry for debugging purposes, but only for this Module.
     * 
    ...
    public void showModule(
            ...
            boolean verbose )
    {
        showModule( mod, 0, verbose, System.err );
    }
            ...
            PrintStream out )
    {
        ...
        if( mod != null ) {
            out.print( mod.getModuleMeta().toString() );
            out.println( " (resolved)" );

            if( verbose ) {
                Module [] dependencies = theForwardDependencies.get( mod );

                for( Module dep : dependencies ) {
                    showModule( dep, indent+1, verbose, out );
                }
            }
        } else {
